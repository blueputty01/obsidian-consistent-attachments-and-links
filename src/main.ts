...
export default class ConsistentAttachmentsAndLinks extends Plugin {
  ...
  async onload() {
    ...
    this.addCommand({
      ...
      editorCallback: (editor: Editor, view: MarkdownView) =>
        this.collectAttachmentsCurrentNote(editor, view),
    });
    ...
    this.settings.ignoreFilesRegex = this.settings.ignoreFiles.map((val) =>
      RegExp(val)
    );
  ...
  async HandleRecentlyRenamedFiles() {
    ...
    try {
      for (let file of this.currentlyRenamingFiles) {
        if (
          this.isPathIgnored(file.newPath) ||
        ...
        if (fileExt == '.md') {
          ...
          if (
            path.dirname(file.oldPath) != path.dirname(file.newPath) ||
            ...
            if (this.settings.moveAttachmentsWithNote) {
              ...
              if (this.settings.updateLinks && result) {
                let changedFiles = result.renamedFiles.concat(
                  result.movedAttachments
                );
                if (changedFiles.length > 0) {
                  await this.lh.updateChangedPathsInNote(
                    file.newPath,
                    changedFiles
                  );
                }
            ...
            if (this.settings.updateLinks) {
              await this.lh.updateInternalLinksInMovedNote(
                file.oldPath,
                file.newPath,
            ...
            if (this.settings.deleteEmptyFolders) {
              if (
                await this.app.vault.adapter.exists(path.dirname(file.oldPath))
              ) {
                let list = await this.app.vault.adapter.list(
                  path.dirname(file.oldPath)
                );
                for (let folder of list.folders) {
                  await this.fh.deleteEmptyFolders(folder);
                }
        ...
        let updateAlts =
          this.settings.changeNoteBacklinksAlt && fileExt == '.md';
        if (this.settings.updateLinks) {
          await this.lh.updateLinksToRenamedFile(
            file.oldPath,
            file.newPath,
            updateAlts,
            this.settings.useBuiltInObsidianLinkCaching
          );
        }
  ...
  async collectAllAttachments() {
    ...
    if (notes) {
      for (let note of notes) {
        if (this.isPathIgnored(note.path)) continue;
  ...
  async convertAllEmbedsPathsToRelative() {
    ...
    if (notes) {
      for (let note of notes) {
        ...
        let result = await this.lh.convertAllNoteEmbedsPathsToRelative(
          note.path
        );
  ...
  async convertAllLinkPathsToRelative() {
    ...
    if (notes) {
      for (let note of notes) {
        ...
        let result = await this.lh.convertAllNoteLinksPathsToRelative(
          note.path
        );
  ...
  async replaceAllWikilinksWithMarkdownLinks() {
    ...
    if (notes) {
      for (let note of notes) {
        ...
        let result = await this.lh.replaceAllNoteWikilinksWithMarkdownLinks(
          note.path
        );
  ...
  deleteEmptyFolders() {
	...
	async checkConsistency() {
		let badLinks = await this.lh.getAllBadLinks();
		let badSectionLinks = await this.lh.getAllBadSectionLinks();
		let badEmbeds = await this.lh.getAllBadEmbeds();
		let wikiLinks = await this.lh.getAllWikiLinks();
		let wikiEmbeds = await this.lh.getAllWikiEmbeds();
    ...
    if (!fileOpened) this.app.workspace.openLinkText(notePath, '/', false);
  }
